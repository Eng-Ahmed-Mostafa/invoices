...
@section('content')
    ...
    <hr class="my-4">
    <form action="{{ route('dashboard.invoices.update',['invoice'=> $invoice->id]) }}" method="POST">
        @method('PUT')
        @csrf
        <div class="row g-3">
            <div class="col">
                <label for="invoice_number">invoice number</label>
                <input id="invoice_number" value="{{ old('invoice_number',$invoice->invoice_number) }}" name="invoice_number" type="text" class="form-control" placeholder="Invoice number" aria-label="Invoice number">
